...
local bool_items = {
	...
	["Safe point"] = {
		references = {({ui_lib.reference("RAGE", "Aimbot", "Safe point")})[1], ({ui_lib.reference("RAGE", "Aimbot", "Safe point")})[2]},
		text = "SP"
	},
	["Freestanding"] = {
...
local custom_items = {
	{
		...
		draw = function(self, x, y, w, h, align_right, text_width, r, g, b, a)
			...
			local chokedcommands_interpolated = math_min(self.chokedcommands_max, self.chokedcommands + (interp < 1 and interp or 0))
			...
			renderer_gradient(bar_x+1, bar_y+1, bar_width*math_max(0, math_min(1, chokedcommands_interpolated / self.chokedcommands_max))-2, bar_h-2, r_top, g_top, b_top, a, r_bot, g_bot, b_bot, a, false)
	...
	},
	{
		...
		get_should_draw = function(self)
			if self.reference == nil then
				...
				self.infinite_duck_reference = ui_lib.reference("AA", "Other", "Infinite duck")
			end
	...
	},
	{
		...
		get_should_draw = function(self)
			if self.reference == nil then
				self.reference, self.hotkey_reference = ui_lib.reference("RAGE", "Aimbot", "Safe point")
			end
			return self.reference:get() and self.hotkey_reference:get()
		end,
		get_text_width = function(self)
